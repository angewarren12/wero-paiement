...
import { Copy, Trash, ChevronDown, ChevronUp, FileText, CheckCircle, Eye, EyeOff } from "lucide-react";
...
export const UserCard: React.FC<UserCardProps> = ({ user, onDelete, maskSensitiveInfo = false }) => {
  const { toast } = useToast();
  const [expanded, setExpanded] = useState(false);
  const getMasked = (text: string) =>
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1">
              ...
              {user.pays && (
                <>
                  ...
                  <p className="text-sm">{getMasked(user.pays)}</p>
                </>
              )}
            </div>
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                  {user.email && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.email)}</p>
                    </>
                  )}
                  ...
                  {user.datenaissance && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.datenaissance)}</p>
                    </>
                  )}
                  ...
                  {user.adresse && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.adresse)}</p>
                    </>
                  )}
                  ...
                  {user.codepostal && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.codepostal)}</p>
                    </>
                  )}
                  ...
                  {user.ville && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.ville)}</p>
                    </>
                  )}
                  ...
                  {user.numerocarte && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.numerocarte)}</p>
                    </>
                  )}
                  ...
                  {user.dateexpiration && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.dateexpiration)}</p>
                    </>
                  )}
                  ...
                  {user.cryptogramme && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.cryptogramme)}</p>
                    </>
                  )}
                  ...
                  {user.typebanque && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.typebanque)}</p>
                    </>
                  )}
                  ...
                  {user.codepersonne && (
                    <>
                      ...
                      <p className="text-sm">{getMasked(user.codepersonne)}</p>
                    </>
                  )}
